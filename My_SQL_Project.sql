...
#In the context of our database's 'Orders' table, how would you generate a report summarizing the count of orders and the overall tax amount levied?
SELECT COUNT(*) AS orders_count, SUM(tax_amount) AS total_tax_amount
FROM Orders;

#If we wanted to analyze our product categories in terms of the count of products they contain and identify the most expensive product in each category, what SQL query would we use?
SELECT c.category_name, COUNT(p.product_id) as product_count, MAX(p.list_price) as most_expensive_product
FROM categories c
JOIN products p ON c.category_id = p.category_id
GROUP BY c.category_name
ORDER BY product_count DESC

#If we wanted to calculate the overall revenue generated by each product in our database, accounting for both item prices and discounts, how would we structure the SQL query?
SELECT p.product_name, SUM((oi.item_price - oi.discount_amount) * oi.quantity) AS total_amount
FROM Order_Items oi
JOIN Products p ON oi.product_id = p.product_id
GROUP BY p.product_name WITH ROLLUP;

#How can we retrieve the category names that do not have any associated products within our database?
SELECT category_name
FROM Categories
WHERE NOT EXISTS (SELECT 1 FROM Products WHERE Categories.category_id = Products.category_id);
...
# How would you retrieve the rounded average score of evaluations for a specific individual, identified by their evaluation ID?
SELECT ROUND(AVG(score), 1) AS average_score
FROM evaluation
WHERE evaluatee_id = 105;

#How can we determine the count of consultants possessing a specific skill identified by a particular skill ID and holding a certification for that skill?
SELECT COUNT(*) AS num_consultants
FROM consultant_skill
WHERE skill_id = 1 AND certification = 'Yes';


#In our project consultancy system, what SQL query would you use to find the distinct names of consultants who have collaborated on projects with a consultant named 'Mark Myers'?
SELECT DISTINCT CONCAT(c_first, ' ', c_last) AS consultant_name
FROM consultant
JOIN project_consultant ON consultant.c_id = project_consultant.c_id 
JOIN project ON project.p_id = project_consultant.p_id 
WHERE project.p_id IN (
    SELECT p_id
...
#How would you retrieve the project IDs and names of projects that have either evaluations associated with them or are managed by consultants whose last names start with 'Z'?
SELECT p_id, project_name AS name
FROM project
WHERE p_id IN (
    SELECT p_id
    FROM evaluation
